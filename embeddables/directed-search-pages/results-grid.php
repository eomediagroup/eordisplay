<?php
/**
 * The template part for displaying an embeddable for 
 * grid layout search results on a Directed Search Page
 *
 */
?>
...
<?php // Let's load all values and labels for ACF select fields
	...
	$dspdisplaytotal = get_field('directed-search-page-results-display-total'); // Total results per page
...
[stratum_results account=<?= $dspaccount ?> listing_url_template="https://eordisplay.teamstratum.com/listing/{{slug}}" num=<?= $dspdisplaytotal ?>]
	[stratum_result_template]
		...
		[stratum_if_value key='featured_image']
			<div class='thumbnail'>[stratum_thumb key='featured_image']</div>
		[/stratum_if_value]
...
<?php echo do_shortcode('[stratum_pagination account=' . $dspaccount .']'); ?>